...
# RTF_DEF, 20190718-weichen-B#17593,[QC23588] TLF stress tests.
# Description: Don't init bdmf history feature which is used for debugging purpose
...
if [ "$board_id" == "3505VW" ] || [ "$board_id" == "3507VW" ] ; then
    bs /bdmf/new system/init_cfg={wan_type=gbe,emac_mode={group_mode=qsgmii,emac4_mode=rgmii,emac5_mode=sgmii},num_wan=1,num_lan=4,enabled_emac=emac0+emac1+emac2+emac3+emac4,wan_emac=emac0,switching_mode=none,ip_class_method=fc},cfg={mtu_size=1536}
else
...
if [ "$board_id" == "3505VW" ] || [ "$board_id" == "3507VW" ] ; then
    bs /bdmf/new egress_tm/dir=ds,index=3,level=queue,mode=sp port/index=lan4
fi

# Configure DS priority queues for LAN emac ports (emac ports schedulers have been already configured)
bs /bdmf/configure egress_tm/dir=ds,index=0 queue_cfg[0]={weight=0,drop_alg=dt,drop_threshold=500,red_high_threshold=0,red_low_threshold=0}
bs /bdmf/configure egress_tm/dir=ds,index=1 queue_cfg[0]={weight=0,drop_alg=dt,drop_threshold=500,red_high_threshold=0,red_low_threshold=0}
bs /bdmf/configure egress_tm/dir=ds,index=2 queue_cfg[0]={weight=0,drop_alg=dt,drop_threshold=500,red_high_threshold=0,red_low_threshold=0}
if [ "$board_id" == "3505VW" ] || [ "$board_id" == "3507VW" ] ; then
    bs /bdmf/configure egress_tm/dir=ds,index=3 queue_cfg[0]={weight=0,drop_alg=dt,drop_threshold=500,red_high_threshold=0,red_low_threshold=0}
fi
