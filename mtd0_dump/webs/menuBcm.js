...
var MENU_OPTION_VOICE_SIP_PBX     =72;
...
function menuAdmin(options) {
   ...
   //Tank_liu,20170718,remove the Internet Time ,START
   //var sntp = options[MENU_OPTION_SNTP];
   var sntp = 0;
   //Tank_liu,20170718,remove the Internet Time ,END
   var ebtables = options[MENU_OPTION_EBTABLES];
   var bridge = options[MENU_OPTION_BRIDGE];
    //Tank_liu,20170718,remove the Parental Control ,START
   //var tod = options[MENU_OPTION_TOD];
   var tod = 0;
   //Tank_liu,20170718,remove the Parental Control ,END
   ...
   var sipPbx = options[MENU_OPTION_VOICE_SIP_PBX];
   ...
   if (anywan) {
      ...
      if ( proto != 'Not Applicable' && ipExt != '1' ) {
         ...
         //insDoc(nodeNat, gLnk('R', getMenuTitle(MENU_SC_HAIRPIN_NAT), 'schairpin.html'));			//RTF_DEF, #14637
         ...
         //nodeIpFlt = insDoc(nodeFirewall, gLnk('R', getMenuTitle(MENU_SC_IP_FILTER), 'scbidirectionalflt.cmd?action=viewcfg'));
         ...
         //insDoc(nodeFirewall, gLnk('R', getMenuTitle(MENU_MAC_FILTER),'scmacflt.cmd?action=view'));
   ...
   if ( parseInt(numWl) != 0 ) {
      ...
      for(i = 0; i < parseInt(numWl); i++)
      {
		 ...
		 if(i==0) //i.e. 2.4GHz part //20180326,Tank_liu F#13746
         {
            insDoc(nodeWireless, gLnk('R', getMenuTitle(MENU_WL_GUEST_SECURITY), 'wlsecurityadvanced.html'));
         }
		 else if (i==1) //i.e. 5G part //20180326,Tank_liu F#13746
		 {
   ...
   if ( voice == 'MGCP' ) {
      ...
      if(sipPbx == '1'){
         insDoc(nodeVoice, gLnk('R', getMenuTitle(MENU_VOICE_SIP_PBX), 'voicesip_pbx.html'));
      }
   }
   ...
   insDoc(nodeSettings, gLnk('R', getMenuTitle(MENU_DEFAULT_REGION), 'defaultRegion.html'));	//RTF_DEF, #14255
   ...
   insDoc(nodeAccCntr, gLnk('R', getMenuTitle(MENU_ACC_CNTR_REMOTE_MANAGEMENT), 'gvtAccessAdministration.html'));

   //B##14945,20180904,Tank
   insDoc(nodeAccCntr, gLnk('R', getMenuTitle(MENU_TRUST_DOMAIN), 'trustDomain.html'));
