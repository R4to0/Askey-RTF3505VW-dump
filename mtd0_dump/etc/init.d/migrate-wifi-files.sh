...
#20190107-weichen-F#16260,[VoIP] Implement new function voip pbx into HGU
   if  [ ! -e /var/asterisk ]
   then
       mkdir -p /var/asterisk
       cp -f /etc/asterisk_def/* /var/asterisk
   fi
#20190107-weichen-F#16260, end

#20190114-chaopo-F#16412,[VoIP] The Asterisk software of HGU IOT test on TEF_SPAIN 
   if  [ ! -e /var/run ]
   then
       mkdir -p /var/run
   fi

   if  [ ! -e /var/run/asterisk ]
   then
       mkdir -p /var/run/asterisk
   fi

   if  [ ! -e /var/lib/asterisk ]
   then
       mkdir -p /var/lib/asterisk
   fi

   if  [ ! -e /var/log/asterisk ]
   then
       mkdir -p /var/log/asterisk
   fi

   if  [ ! -e /tmp/log/asterisk ]
   then
       mkdir -p /tmp/log/asterisk
   fi

   if  [ ! -e /var/spool/asterisk ]
   then
       mkdir -p /var/spool/asterisk
   fi

   if  [ ! -e /var/spool/asterisk/voicemail ]
   then
       mkdir -p /var/spool/asterisk/voicemail
   fi

   if  [ ! -e /var/lib/asterisk/keys ]
   then
       mkdir -p /var/lib/asterisk/keys
   fi

# RM19815 chaopo 20200413 new requirement for call transfer
#RTF_DEF_VOIP_PBX_AST_XFER_MEDIA_USED not used
#   if  [ ! -e /var/lib/asterisk/sounds/en ]
#   then
#       mkdir -p /var/lib/asterisk/sounds/en
#       cp -f /usr/lib/asterisk/sounds/en/* /var/lib/asterisk/sounds/en
#   fi
#20190107-chaopo-F#16412, end
fi
...
# 20160930-weichen-#9646, end
    ...
    #20210820-weichen-F#24001,[OPA-H][Askey]Script / Method for managing pause and block of several devices
    echo "cp TEF script files from /bin/ to /usr/local/auxfs/devices/"
    mkdir -p /usr/local/auxfs/devices
    cp -a /bin/*Device*.sh /usr/local/auxfs/devices/
    #20210820-weichen-F#24001, end

    echo "copy /etc/rtf_version to auxfs"
    cp -a /etc/rtf_version /usr/local/auxfs/

    echo "fw_upgrade,$(( $(( $(date +"%M")*60 + $(date +"%S") )) ))" > /data/reboot_info
